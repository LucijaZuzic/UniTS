...
def find_match(transformed_pd_file, transformed_xs_file, transformed_true_file, transformed_pred_file, var_name):
    ...
    for ix_xs in range(len(transformed_xs_file)):
        dict_xs_pred_key = tuple(np.round(transformed_xs_file[ix_xs], use_ndec))
    ...
    print(doubled, len(transformed_true_file), np.round(doubled / len(transformed_true_file) * 100, 2))
    ...
    for ix_match in range(len(transformed_pd_file)):
        dict_xs_pred_key = tuple(np.round(transformed_pd_file[ix_match], use_ndec))
        if dict_xs_pred_key not in dict_xs_pred_true:
            notfound += 1
            gap = ix_match - lastix
    ...
    print(notfound, len(transformed_pd_file), np.round(notfound / len(transformed_pd_file) * 100, 2))
        ...
        for s1 in first_sufix:
            for s2 in second_sufix:
                for t in types_used:
                    ...
                    find_match(pd_file_val_transformed, xs_val, trues_val, preds_val, varname)